...
const subject="LMS HEXA";
  ...
  const sendEmail = (e) => {
    ...
    axios.get('https://localhost:44312/api/Manager/GetByEmail/' +email)
    .then(res => res
    )
    .then(result => {
      let r=result.data
      sessionStorage.setItem("manager_id", result.data.manager_Id);

      if(r!=null){
          alert("account found");
          emailjs.sendForm('service_864xkci', 'template_dd7vupc', form.current, 'Du94QdqS3eYRm7br5')
          .then((result) => {
...
//  alert("session"+potp+"eotp"+eotp);
                  if(notp==potp){
                  window.location="/managerforgotpassword"
                       }
                  else{
...
export default EnterEmailManager